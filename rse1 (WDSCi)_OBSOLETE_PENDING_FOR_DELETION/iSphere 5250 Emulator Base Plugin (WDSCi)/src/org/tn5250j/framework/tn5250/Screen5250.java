...
public class Screen5250 implements TN5250jConstants {
    ...
    public final void copyMe(Rectangle area) {
        ...
        // }
        // StringSelection contents = new StringSelection(trString +
        // s.toString());
        ...
        try {
            ...
            for (int x = nextChar; x < nChars; x++) {
                ...
                if ((c == '\n') || (c == '\r')) {
                    ...
                    // to the starting column of the next row and start from
                    // there
                    cpos = getPos(getRow(cpos) + 1, lc);

                    // If we go paste the end of the screen then let's start
                    // over from
                    ...
                    // If we are in a special paste scenario then we check for
                    // valid
                    // characters to paste.
                    if (special && (!Character.isLetter(c) && !Character.isDigit(c))) setIt = false;

                    // we will only push a character to the screen space if we
                    // are in
                    // a field
            ...
            if (g >= BUTTON_LEFT && g <= BUTTON_LAST) {
                ...
                if (aidFlag) {
                    ...
                    case BUTTON_SB_GUIDE:
                        sessionVT.sendAidKey(TN5250jConstants.AID_ROLL_UP);
                        break;
                    ...
                    case BUTTON_SB_THUMB:
                        ...
                        sessionVT.sendAidKey(TN5250jConstants.AID_ROLL_DOWN);
                        break;
                    case BUTTON_LEFT_EB:
                    ...
                    if (screenFields.getCurrentField() != null) {
                        ...
                        screenFields.setCurrentFieldMDT();
                        sessionVT.sendAidKey(TN5250jConstants.AID_ENTER);
                    }
    ...
    public int getRow(int pos) {
        ...
        if (row > lenScreen - 1) row = lenScreen - 1;
    ...
    private void process_XY(int pos) {
        ...
        if (feError) {
            ...
            if (screenFields.getCurrentField() != null && screenFields.getCurrentField().isFER() && screenFields.getCurrentFieldPos() - 1 == pos) {
            }
        } else {
            goto_XY(pos);
        }
