...
public class DataStreamProducer implements Runnable {
    ...
    public final byte[] readIncoming() throws IOException {
        ...
        while (!done) {
            ...
            if (j == 255 && i == 255) {
                ...
                // we will negotiate with the server by sending a WONT'T
                // TIMING-MARK
                // This will let the server know that we processed the
                // information
                // and are just waiting for the user to enter some data so keep
                // the
                // socket alive. This is more or less a AYT (ARE YOU THERE) or
                // not.
                if (i == 253 && j == 255) {
                    done = true;
                    negotiate = true;
    ...
    // System.out.println(i + ">" + shit[i] + "< - ascii - >" +
    // getASCIIChar(shit[i]) + "<");
    // }
    //
    ...
    // byte shit[] = abyte;
    // for (int i = 0;i < shit.length;i++)
    // System.out.println(i + ">" + shit[i] + "< hex >" +
    // Integer.toHexString((shit[i] & 0xff)));
    // }
