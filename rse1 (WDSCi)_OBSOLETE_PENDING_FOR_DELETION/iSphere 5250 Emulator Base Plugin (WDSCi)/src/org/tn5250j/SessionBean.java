...
public class SessionBean extends SessionGUI {
    ...
    public void signoff() {
        if (session.getVT() != null) {
            if (this.isConnected()) this.session.getVT().systemRequest("90");
        }
    }
    ...
    public void doLayout() {
        ...
        if (prevRect == null) {
            prevRect = rect;
            this.resizeMe(); // necessary when it is the first time: compute
                                // the fontsize
            return;
        }
    ...
    public Dimension getPreferredSize() {
        if (preferredSize == null) this.setPreferredSize(SessionBean.deriveOptimalSize(this, this.getFont(), 80, 24));
    ...
    private void doAfterSignon() {
        if (isSignificant(afterSignon)) this.getScreen().sendKeys(afterSignon);
    }
    ...
    private void failIfConnected() {
        if ((session.getVT() != null) && (isConnected())) throw new IllegalStateException("Cannot change property after being connected!");
    }
