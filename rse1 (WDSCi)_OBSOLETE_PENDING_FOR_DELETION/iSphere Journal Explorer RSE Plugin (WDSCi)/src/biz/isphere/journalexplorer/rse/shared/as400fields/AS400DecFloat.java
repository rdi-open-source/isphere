...
import biz.isphere.journalexplorer.rse.as400.access.SQLDataFactory;
...
//@DFA new class
/**
 * The AS400DecFloat class provides a converter between a BigDecimal object and
 * a DecimalFloat type.
 */
public class AS400DecFloat implements AS400DataType {
    ...
    private final static long DEC_FLOAT_16_SIGNAL_MASK = 0x0200000000000000L; // 1
    // bit
    // (7th
    // bit
    // from
    // left)
    // //@snan
    private final static long DEC_FLOAT_16_SIGN_MASK = 0x8000000000000000L; // 1
    // bits
    private final static long DEC_FLOAT_16_COMBINATION_MASK = 0x7c00000000000000L; // 5
    // bits
    private final static long DEC_FLOAT_16_EXPONENT_CONTINUATION_MASK = 0x03fc000000000000L; // 8
    // bits
    private final static long DEC_FLOAT_16_COEFFICIENT_CONTINUATION_MASK = 0x0003ffffffffffffL; // 50
    // bits

    private final static int DEC_FLOAT_34_BIAS = 6176;
    private final static long DEC_FLOAT_34_SIGNAL_MASK = 0x0200000000000000L; // 1
    // bit
    // (7th
    // bit
    // from
    // left)
    // //@snan
    private final static long DEC_FLOAT_34_SIGN_MASK = 0x8000000000000000L; // 1
    // bits
    private final static long DEC_FLOAT_34_COMBINATION_MASK = 0x7c00000000000000L; // 5
    // bits
    private final static long DEC_FLOAT_34_EXPONENT_CONTINUATION_MASK = 0x03ffc00000000000L; // 12
    // bits
    final static long DEC_FLOAT_34_COEFFICIENT_CONTINUATION_MASK = 0x00003fffffffffffL; // 46
    // bits
    // + 64
    // bits
    // =
    ...
    public AS400DecFloat(int numDigits) {
        ...
        {
            throw new ExtendedIllegalArgumentException("numDigits (" + String.valueOf(numDigits) + ")",
    ...
    /**
     * Creates a new AS400DecFloat object that is identical to the current
     * instance.
     * 
    ...
    public int getInstanceType() {
        // From "com.ibm.as400.access.AS400DataType"
     ...
     * 
     * @param javaValue The object corresponding to the data type. It must be an
     *        instance of BigDecimal and the BigDecimal must have a less than or
    ...
    /**
     * Converts the specified Java object into server format in the specified
     * byte array.
     * 
     * @param javaValue The object corresponding to the data type. It must be an
     *        instance of BigDecimal and the BigDecimal must have a less than or
     ...
     * byte array.
     * 
     * @param javaValue An object corresponding to the data type. It must be an
     *        instance of BigDecimal or String (if value is "NaN", "Infinity",
     *        or "-Infinity").
     * @param as400Value The array to receive the data type in server format.
     *        There must be enough space to hold the server value.
     * @param offset The offset into the byte array for the start of the server
     *        value. It must be greater than or equal to zero.
     * @return The number of bytes in the server representation of the data
     *         type.
     */
    public int toBytes(Object javaValue, byte[] as400Value, int offset) {
        // verify input
        long specialCombination = 0L; // ieee algorithm says: Combination G
        // (11111-> NaN, 11110-> (-1)^sign
        // Infinity)
        int signalingNaN = -1; // @sig1 for now, only support non-signaling
        // until decfloat/double etc support it
        if (javaValue instanceof String) {
            // special value "NaN", "Infinity", or "-Infinity"
            // use dummy BigDecimal("1" or "-1"), and overlay ieee (Combination
            // G) at end of method
            ...
            } else if (javaValue.equals("-SNaN")) // @snan
            {
                ...
                javaValue = new BigDecimal("-1"); // negative dummy so that
                // sign gets set to negative
                // Infinity
                specialCombination = 0x1eL;
            }

        }
        BigDecimal inValue = (BigDecimal)javaValue; // Let this line throw
        // ClassCastException
        ...
        if (this.digits == 16) // DECFLOAT16
        {
            ...
            int bdPrecision = SQLDataFactory.getPrecisionForTruncation(inValue, 16)[0]; // bdUnscaledStr.length
            // ();
            // //@rnd1

            // bug in jdk1.5 (need to pad with 0z if exp is greater than (maxexp
            // - (maxprecision - precision)) (ie. 9.99E380 since 380>384-(16-3))
            // //@max
            // so if among the top 16 exponent values, then need to have the
            // precision digits padded with zeros
            int zeros = 0; // @max
            if ((exponent > 368) && bdUnscaledStr.length() < 16) { // maxexp-16
                // //@max
                // pad 0s //@max
            ...
            if (bdUnscaledStr.length() > bdPrecision) exponent = bdUnscaledStr.length() - bdPrecision; // get
            // exponent
            // in
            // terms
            // of
            ...
            if ((exponent + (bdPrecision - 1)) > 384)
                throw new ExtendedIllegalArgumentException("numDecimalPositions (" + String.valueOf((exponent + (bdPrecision - 1))) + ")",
                    ExtendedIllegalArgumentException.RANGE_NOT_VALID);
            ...
            for (int indx = 1; indx < 16; indx += 3) {
                decFloat16Bits <<= 10; // declet. (3 digits are accommondated
                // in 10 bits).
            ...
            if (specialCombination != 0L) {
                combination = specialCombination; // set for Combination G
                // (11111-> NaN, 11110->
                // (-1)^sign Infinity)
            ...
            if (signalingNaN == 0) // @sig1
                as400Value[offset] &= 0xFD; // non signaling (switch off 7th
            // bit) //@sig1
            else if (signalingNaN == 1) // @sig1
                as400Value[offset] |= 0x02; // signaling (switch on 7th bit)
            // //@sig1
        ...
        } else // DECFLOAT34
        {
            ...
            // get precision of the BigDecimal.
            int bdPrecision = SQLDataFactory.getPrecisionForTruncation(inValue, 34)[0]; // bdUnscaledStr.length
            // ();
            // //@rnd1

            // bug in jdk1.5 //@max
            int zeros = 0; // @max
            if ((exponent > 6110) && bdUnscaledStr.length() < 34) { // maxexp-34
                // //@max
                // pad 0s //@max
            ...
            for (; indx < 13; indx += 3) {
                decFloat34BitsHi <<= 10; // 3 digits are accommondated in 10
                // bits.
            ...
            decDigits = packDenselyPackedDecimal(coefficientDigits, indx);
            decFloat34BitsHi <<= 6;
            decFloat34BitsHi |= ((decDigits & 0x3f0) >> 4); // get high 6 bits
            // of decDigits.
            decFloat34BitsLo |= (decDigits & 0xf); // get low 4 bits of
            // decDigits.
            indx += 3;
            // handle the rest 18 digits in high 8 bytes.
            for (; indx < 34; indx += 3) {
                decFloat34BitsLo <<= 10; // 3 digits are accommondated in 10
                // bits.
                decDigits = packDenselyPackedDecimal(coefficientDigits, indx);
     ...
     * Converts the specified Java object to server format.
     * 
     * @param doubleValue The value to be converted to server format. If the
     *        decimal part of this value needs to be truncated, it will be
     *        rounded based on decfloat rounding mode property.
     * @return The server representation of the data type.
     */
    ...
    public double toDouble(byte[] as400Value, int offset) {
        ...
        if (offset < 0) throw new ArrayIndexOutOfBoundsException(String.valueOf(offset));
     ...
     * 
     * @param as400Value The array containing the data type in server format.
     *        The entire data type must be represented and the data type must
     *        have valid packed decimal format.
     * @param offset The offset into the byte array for the start of the server
     *        value. It must be greater than or equal to zero.
     * @return The BigDecimal object corresponding to the data type.
     */
    public Object toObject(byte[] as400Value, int offset) {
        ...
        if (this.digits == 16) {
            ...
            if ((combination == 0x1fL) && (sign == 1)) {
                long nanSignal = (decFloat16Bits & DEC_FLOAT_16_SIGNAL_MASK) >> 57; // shift
                // first
                // 7
                // bits
                // to
            ...
            int coefficientLo = decFloatBitsToDigits((int)(coefficientContinuation & 0x3fffffff)); // low
            // 30
            // bits
            // (9
            // digits)
            int coefficientHi = decFloatBitsToDigits((int)((coefficientContinuation >> 30) & 0xfffff)); // high
            // 20
            // bits
            // (6
            // digits)
            ...
            if ((combination == 0x1fL) && (sign == 1)) {
                long nanSignal = (decFloat34BitsHi & DEC_FLOAT_34_SIGNAL_MASK) >> 57; // shift
                // first
                // 7
                // bits
                // to
            ...
            // compute the coefficient.
            int coefficientLo = decFloatBitsToDigits((int)(decFloat34BitsLo & 0x3fffffff)); // last
            // 30
            // bits
            // (9
            // digits)
            ...
            int coefficientHi = decFloatBitsToDigits((int)((decFloat34BitsHi >> 26) & 0xfffff)); // high
            // 20
            // bits
            // (6
            // digits)
    ...
    // */
    // static final void throwNumberFormatException(boolean highNibble, int
    // byteOffset, int byteValue, byte[] fieldBytes) throws
    // NumberFormatException
    // {
    // AS400PackedDecimal.throwNumberFormatException(highNibble, byteOffset,
    // byteValue, fieldBytes);
    // }
    ...
    private static int packDenselyPackedDecimal(int[] digits, int indx) {
        // Declet is the three bit encoding of one decimal digit. The Decfloat
        // is made up of declets to represent
    ...
    private static final int[] computeMagnitude(int[] input) {
        ...
        for (int i = 0; i < length - 1; i++) {
            ...
            int k = length - 1;
            for (; j >= 0; j--, k--) {
                long product = (input[length - 2 - i] & 0xFFFFFFFFL) * (tenRadixMagnitude[i][j] & 0xFFFFFFFFL) + (mag[k] & 0xFFFFFFFFL) // add
                    // previous
                    // value
    ...
    /**
     * Convert 30 binary bits coefficient to 9 decimal digits. Note that for
     * performance purpose, it does not do array-out-of-bound checking.
     */
     ...
     * mcPrecision and mcRoundingMode are what is in jdk 5.0 MathContext. What
     * is returned from this method should be the same as what jre 5.0 would
    ...
    private static BigDecimal roundByModePreJDK5(BigInteger intVal, int scale, int mcPrecision, String mcRoundingMode) {
        ...
        if (mcPrecision == 16)
            ...
            roundingMax = new BigInteger("10000000000000000000000000000000000"); // 34
        // 0s

        BigInteger roundingMin = roundingMax.negate();

        if (roundingMax != null && intVal.compareTo(roundingMax) < 0 && intVal.compareTo(roundingMin) > 0) return getNewBigDecimal(intVal, scale); // rounding
        // not
        // needed
        // get precision from intVal without 0's on right side
        int[] values = SQLDataFactory.getPrecisionForTruncation(getNewBigDecimal(intVal, scale), mcPrecision); // =precisionStr.length()
        // -
        // trimCount;
        // //@rnd1
        int precisionNormalized = values[0]; // @rnd1
        int droppedZeros = values[1]; // @rnd1 decrease scale by number of
        // zeros removed from precision //@rnd1
    ...
    private static BigDecimal roundOffDigits(BigInteger intVal, int scale, String mcRoundingMode, int dropCount) {
        ...
        try {
            ...
            throw new InternalErrorException(InternalErrorException.UNKNOWN); // should
            // never
            // happen
        }
        BigDecimal rounded = preRoundedBD.divide(divisor, scale, roundingMode); // do
        // actual
        // rounding
        // here
    ...
    /**
     * Creates and returns a new BigDecimal based on parameters. This is a
     * temporary hack due to pre-jre 1.5 not being able to handle negative
     * scales (positive exp) After we no longer support pre-java 1.5, this
     * method can be replaced with new BigDecimal(bigInt, scale).
     * 
    ...
    private static BigDecimal getNewBigDecimal(BigInteger bigInt, int scale) {
        ...
        try {
            ...
            // note that creating BigDecimal with negative scale is ok in 5, but
            // not in 1.4
    ...
    // Decimal float. //@DFA
    /**
     * Rounds the precision of a BigDecimal by removing least significant digits
     * from the right side of the precision. (least significant digits could be
     * left or right of implicit decimal point)
     * 
    ...
    public static BigDecimal roundByMode(BigDecimal bd, int precision, String roundingMode) {
        ...
        // MathContext is in jdk1.5. So use reflection so code will build under
        // pre-1.5
        // later, use this when we move to jdk1.5
        // All we are doing below is: bdAbs = inValue.abs(new MathContext(16,
        // roundingMode));
        boolean isGEJVM50 = true;
        try {
            // in this try block, we do rounding via BigDecimal and MathContext
            Class cls = Class.forName("java.math.MathContext"); // thorw
            // ClassNotFoundException
            // if pre 1.5
            // jvm
            Constructor ct = cls.getConstructor(new Class[] { Integer.TYPE, Class.forName("java.math.RoundingMode") });
            Object arglist[] = new Object[2];
            arglist[0] = new Integer(precision); // MathContext.DECIMAL64 (16
            // or 34 char decfloat
            // precision)
            arglist[1] = Class.forName("java.math.RoundingMode").getDeclaredField(roundingMode.substring(6)).get(null); // @pdc
            // remove
            // "ROUND_"

            Object mathContextRounded = ct.newInstance(arglist); // ie. new
            // MathContext(16or34,
            // RoundingMode.x);
        ...
        // Unfortunately, we cannot just catch Exception since we do not want to
        // miss any real exceptions
        // from rounding etc. And can't re-throw Exception it since method is
