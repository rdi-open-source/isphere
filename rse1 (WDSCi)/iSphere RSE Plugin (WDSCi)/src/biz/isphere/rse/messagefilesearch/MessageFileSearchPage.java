...
import biz.isphere.core.internal.DisplayMessage;
...
public class MessageFileSearchPage extends DialogPage implements ISearchPage, Listener {
    ...
    /**
     * Retrieves a screen value of the last search.
     * 
     ...
     *        yet exist
     * @return the screen value of the last search
     */
    private String loadValue(String aKey, String aDefault) {
    ...
    /**
     * Stores a given screen value to preserve it for the next search.
     * 
    ...
    /**
     * Stores a given numeric screen value to preserve it for the next search.
     * 
     * @param aKey - key, the value is assigned to
     * @param aValue - the screen value that is stored
     */
    private void storeValue(String aKey, int aValue) {
        getDialogBoundsSettings().put(aKey, aValue);
    }
    ...
    public boolean performAction() {
        ...
        try {
            ...
            new SearchExec().execute(tConnection.getAS400ToolboxObject(getShell()), tConnection.getHostName(), tConnection.getJDBCConnection(null,
                false), getSearchString(), startColumn, endColumn, getCase(), new ArrayList<SearchElement>(searchElements.values()), postRun);
