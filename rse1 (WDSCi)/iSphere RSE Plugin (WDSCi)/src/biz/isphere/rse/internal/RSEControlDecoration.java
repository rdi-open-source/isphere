...
/**
 * iSphere Team: Backport from Eclipse 3.6 (Helios).
 * <p>
 ...
 */
public class RSEControlDecoration implements IControlDecoration {
    /**
...
//    private static boolean MAC = Util.isMac();
    ...
    /**
     * The rectangle of the previously used image. Used for redrawing in the
     * case where a smaller image replaces a larger one.
     * 
     * @since 3.5
     */
    ...
    class Hover {
        ...
        int[] getPolygon(boolean border) {
            ...
            int b = border ? 1 : 0;
            if (arrowOnLeft) {
                return new int[] { 0, 0, e.x - b, 0, e.x - b, e.y - b, hao + haw, e.y - b, hao + haw / 2, e.y + hah - b, hao, e.y - b, 0, e.y - b, 0,
                    0 };
            }
            return new int[] { 0, 0, e.x - b, 0, e.x - b, e.y - b, e.x - hao - b, e.y - b, e.x - hao - haw / 2, e.y + hah - b, e.x - hao - haw,
                e.y - b, 0, e.y - b, 0, 0 };
        }
     ...
     *        rendered. The decoration will be clipped to this composite, but it
     *        may be rendered on a child of the composite. The decoration will
     *        not be visible if the specified composite or its child composites
     *        are not visible in the space relative to the control, where the
     *        decoration is to be rendered. If this value is <code>null</code>,
     *        then the decoration will be rendered on whichever composite (or
     *        composites) are located in the specified position.
     */
...
//    }
     ...
     * 
     * @param listener the listener which should no longer be notified. This
     *        message has no effect if the listener was not previously added to
     *        the receiver.
     * 
     * @see org.eclipse.swt.events.MenuDetectListener
...
//    }
    ...
    private void addControlListeners() {
        ...
        compositeListener = new Listener() {
            public void handleEvent(Event event) {
                ...
                switch (event.type) {
                case SWT.MouseDown:
                    if (!selectionListeners.isEmpty()) notifySelectionListeners(event);
                    break;
     ...
     * 
     * @param text the text to be shown in the info hover, or <code>null</code>
     ...
     * decoration.
     * 
     * @param text the text to be shown as a description for the decoration, or
     *        <code>null</code> if none has been set.
     */
     ...
     * decoration.
     * 
     * @param image the image to be shown adjacent to the control. Should never
     *        be <code>null</code>.
     */
     ...
     * <code>false</code>.
     * 
     * @param showOnlyOnFocus <code>true</code> if the decoration should only
     *        be shown when the control has focus, and <code>false</code> if
     *        it should always be shown. Note that if the control is not capable
     *        of receiving focus (<code>SWT.NO_FOCUS</code>), then the
     *        decoration will never show when this value is <code>true</code>.
     */
     ...
     * default value of this setting is <code>true</code>.
     * 
     * @param showHover <code>true</code> if a hover popup containing the
     *        decoration's description text should be shown when the user hovers
     *        over the decoration, and <code>false</code> if a hover should
     *        not be shown.
     */
     ...
     * is <code>0</code>.
     * 
     * @param marginWidth the number of pixels that should be reserved between
     *        the horizontal edge of the control and the adjacent edge of the
     *        decoration.
     */
    ...
    private void showHoverText(String text, Control hoverNear) {
        ...
        // invisible.
        if (control.isDisposed() || !control.isVisible()) return;
     ...
     * method considers the visibility state of the decoration ( {@link #hide()}
     * and {@link #show()}), the visibility state of the associated control ({@link Control#isVisible()}),
     * and the focus state of the control if applicable ({@link #setShowOnlyOnFocus(boolean)}.
     * When this method returns <code>true</code>, it means that the
     * decoration should be visible. However, this method does not consider the
     * case where the decoration should be visible, but is obscured by another
     * window or control, or positioned off the screen. In these cases, the
     * decoration will still be considered visible.
     * 
