...
class BasicScreen extends BasicSubUI implements ScreenListener, TN5250jConstants {
    ...
    /**
     * Holds row and column coordinates. An iOhioPosition can be constructed
     * using initial row and column coordinates or constructed with no values
     * and have the row and column set later.
     */
    ...
    public final class iOhioPosition {
         ...
         */
        public iOhioPosition(int initRow, int initCol) {
    ...
    // public void onScreenChanged(int inUpdate, iOhioPosition inStart,
    // iOhioPosition inEnd)
...

    {
        ...
        // addDirtyRectangle(cs.x, cs.y, ce.x - cs.x + columnWidth, ce.y - cs.y
        // + rowHeight);
        // break;
        }
    }
    ...
    /**
     * PRE: The x,y coordinate must be between the innerbounds of the screen!
     */
    ...
    protected void paintCursor(Graphics g) {
        ...
        // // System.out.println("Cursor at " + pos.getColumn() + "," +
        // pos.getRow());
        Rectangle r = modelToView(pos);
    ...
    protected void paintPositions(Graphics g, int x, int y, iOhioPosition start, iOhioPosition end) {
        ...
        // //y += metrics.getAscent();
        //
        // for (int row = start.getRow(), rowEnd = end.getRow(); row < rowEnd;
        // row++)
        // {
            ...
            if (useGui && (whichGui < FIELD_LEFT)) {
                ...
                if (underLine) {
                    ...
                    if (!useGui) {
                        ...
                        // g.drawLine(x, (int)(y + (rowHeight -
                        // s.lm.getLeading()-5)), (int)(x + columnWidth),
                        // (int)(y + (rowHeight - s.lm.getLeading())-5));
                        g.drawLine(x, (int)(y + (rowHeight - (lm.getLeading() + lm.getDescent()))), (x + columnWidth), (int)(y + (rowHeight - (lm
                            .getLeading() + lm.getDescent()))));
                    }
                }
                ...
                if (colSep) {
                    ...
                    // g.drawLine(x + columnWidth - 1, y, x + columnWidth - 1, y
                    // + rowHeight);
                    // break;
                    // case 1: // short line
                    // g.drawLine(x, y + rowHeight - (int)lm.getLeading()-4, x,
                    // y + rowHeight);
                    // g.drawLine(x + columnWidth - 1, y + rowHeight -
                    // (int)lm.getLeading()-4, x + columnWidth - 1, y +
                    // rowHeight);
                    // break;
                    // case 2: // dot
                    // g.drawLine(x, y + rowHeight - (int)lm.getLeading()-3, x,
                    // y + rowHeight - (int)lm.getLeading()-4);
                    // g.drawLine(x + columnWidth - 1, y + rowHeight -
                    // (int)lm.getLeading()-3, x + columnWidth - 1, y +
                    // rowHeight - (int)lm.getLeading()-4);
                    // break;
                    // case 3: // hide
                    // break;
                    // }
    ...
    // protected int paintRow(Graphics g, int x, int y, Data data, int offset,
    // int length)
    // {
    ...
    // protected int paintPosition(Graphics g, int x, int y, Data data, int
    // offset)
    // {
    ...
    // {
    // //int uline = y - metrics.getAscent() + metrics.getHeight() -
    // metrics.getDescent();
    ...
    // {
    // // return new Data( screen.getData(start, end,
    // iOhioScreen.OHIO_PLAIN_TEXT)
    ...
    private Rectangle adjustRectangle(Rectangle region) {
        if (region == null) return this.getBounds();
        ...
        if (off > 0) adjs.width += columnWidth - off;

        off = rowOffset(adjs.y + adjs.height);
        if (off > 0) adjs.height += rowHeight - off;
