...
public class Gui5250 extends JPanel implements ComponentListener, ActionListener, TN5250jConstants, RubberBandCanvasIF,
// SessionListener,
    ...
    private void jbInit() throws Exception {
            ...
            public void mouseClicked(MouseEvent e) {
                ...
                if (e.getClickCount() == 2 & doubleClick) {
                    ...
                    if (log.isDebugEnabled()) {
                        ...
                        log.debug(e.getX() + "," + e.getY() + "," + bi.columnWidth + "," + bi.rowHeight);
                    }
        ...
        if (!sesConfig.getStringProperty("mouseWheel").equals("Yes")) scroller.removeMouseWheelListener((SessionGUI)this);
    ...
    private void actionTransfer(java.io.File file) {
        ...
        try {
            ...
            org.tn5250j.tools.XTFRFile tfr = new org.tn5250j.tools.XTFRFile(null, session.getVT(), (SessionGUI)this, props);
        } catch (Exception exc) {
    ...
    public void doKeyBoundArea(KeyEvent ke, String last) {
        ...
        if (!rubberband.isAreaSelected()) {
            ...
            // cursor
            // is now.
            if (last.equals("[markleft]")) bi.getPointFromRowCol(screen.getCurrentRow() - 1, screen.getCurrentCol() + 1, p);
            // mark right will mark the current position to the left of the
            // current cursor position
            if (last.equals("[markright]")) bi.getPointFromRowCol(screen.getCurrentRow() - 1, screen.getCurrentCol() - 2, p);

            if (last.equals("[markup]")) bi.getPointFromRowCol(screen.getCurrentRow() + 1, screen.getCurrentCol() - 1, p);
            // mark down will mark the current position minus the current
            // row.
            if (last.equals("[markdown]")) bi.getPointFromRowCol(screen.getCurrentRow() - 2, screen.getCurrentCol() - 1, p);
        ...
        bi.getPointFromRowCol(screen.getCurrentRow() - 1, screen.getCurrentCol() - 1, p);
    ...
    protected void fireSessionJump(int dir) {
        ...
        if (listeners != null) {
            ...
            for (int i = 0; i < size; i++) {
                SessionJumpListener target = (SessionJumpListener)listeners.elementAt(i);
    ...
    protected void fireEmulatorAction(int action) {
        ...
        if (actionListeners != null) {
            ...
            for (int i = 0; i < size; i++) {
                EmulatorActionListener target = (EmulatorActionListener)actionListeners.elementAt(i);
    ...
    public void actionSpool() {
        ...
        try {
            org.tn5250j.spoolfile.SpoolExporter spooler = new org.tn5250j.spoolfile.SpoolExporter(session.getVT(), (SessionGUI)this);
    ...
    public void resizeMe() {
        ...
        if (bi != null) bi.resizeScreenArea(r.width, r.height);
        screen.repaintScreen();
        Graphics g = getGraphics();
        if (g != null) g.setClip(0, 0, this.getWidth(), this.getHeight());
    ...
    public void crossHair() {
        ...
        bi.crossHair++;
        if (bi.crossHair > 3) bi.crossHair = 0;
    ...
    public final void printMe() {
        ...
        Thread printerThread = new PrinterThread(screen, bi.font, screen.getColumns(), screen.getRows(), Color.black, true, (SessionGUI)this);
