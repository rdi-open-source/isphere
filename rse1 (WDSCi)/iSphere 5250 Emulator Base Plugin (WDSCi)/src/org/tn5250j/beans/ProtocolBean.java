...
public class ProtocolBean implements TN5250jConstants {
    ...
    public ProtocolBean(String configurationResource, String sessionName) {
        this(new Properties(), configurationResource, sessionName);
    }

    public ProtocolBean(Properties sessionProperties, String configurationResource, String sessionName) {

        session = new Session5250(sessionProperties, null, sessionName, new SessionConfig(configurationResource, sessionName));
    ...
    public void setIPAddress(String ipAddress) throws IllegalStateException {
        ...
        if (isSignificant(ipAddress)) sessionProperties.put(SESSION_HOST, ipAddress);
    }

    public void setIPPort(int ipPort) {
        failIfConnected();

        if (ipPort < 0) throw new IllegalArgumentException("An IP port must be greate or equal to 0!");
    ...
    public void setCodePage(String charEncoding) {
        ...
        if (isSignificant(charEncoding)) {
            if (charEncoding.startsWith("Cp")) charEncoding = charEncoding.substring(2);
    ...
    public void setDeviceName(String deviceName) {
        ...
        if (isSignificant(deviceName)) sessionProperties.put(SESSION_DEVICE_NAME, deviceName);
    }
    ...
    public void connect() {
        ...
        if (!isSignificant(this.user)) {
            ...
            Runnable runnable = new Runnable() {
                ...
                public void run() {
                    if ((tryConnection++ < 30) && // If it is still not
                                                    // connected after 3
                                                    // seconds,
                        // stop with trying
    ...
    public void signoff() {
        if (session.getVT() != null) {
            if (session.isConnected()) session.getVT().systemRequest("90");
        }
    }
    ...
    private void connectEmbedded() {
        ...
        if (isSignificant(user)) sessionProperties.put("SESSION_CONNECT_USER", user);

        if (password != null) sessionProperties.put("SESSION_CONNECT_PASSWORD", password);

        if (isSignificant(program)) sessionProperties.put("SESSION_CONNECT_PROGRAM", program);

        if (isSignificant(menu)) sessionProperties.put("SESSION_CONNECT_MENU", menu);

        if (isSignificant(library)) sessionProperties.put("SESSION_CONNECT_LIBRARY", library);
    ...
    private void connectSimulated() {
        ...
        if (isSignificant(user)) sb.append(user);
        if (!isFieldLength(user)) sb.append(MNEMONIC_TAB);

        if (isSignificant(password)) sb.append(password);
        if (!isFieldLength(password)) sb.append(MNEMONIC_TAB);
        ...
        if (isSignificant(program) || isSignificant(menu) || isSignificant(library)) {
            if (isSignificant(program)) sb.append(program);
            if (!isFieldLength(program)) sb.append(MNEMONIC_TAB);

            if (isSignificant(menu)) sb.append(menu);
            if (!isFieldLength(menu)) sb.append(MNEMONIC_TAB);

            if (isSignificant(library)) sb.append(library);
        }
    ...
    private void doAfterSignon() {
        if (isSignificant(afterSignon)) session.getScreen().sendKeys(afterSignon);
    }
    ...
    private void failIfNot10(String param) {
        if ((param != null) && (param.length() > 10)) throw new IllegalArgumentException("The length of the parameter cannot exceed 10 positions!");
    }
