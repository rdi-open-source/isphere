...
public class DisplayJournalEntriesHandler {
	...
	public static void handleDisplayJournalEntries(
			ISelectedJournal... selectedJournals) {
	...
	public static void handleDisplayFileJournalEntries(
			ISelectedFile... selectedFiles) {
		...
		factory.handleDisplayFileJournalEntries(selectedFiles);
	}
	...
	private static IDisplayJournalEntriesContributions getContributionsFactory() {
		...
		if (factory == null) {
			...
			IConfigurationElement[] configElements = tRegistry
					.getConfigurationElementsFor(EXTENSION_ID);

			if (configElements != null && configElements.length > 0) {
				try {
					final Object tempDialog = configElements[0]
